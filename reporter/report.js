...
for (const col of columns) {
  ...
  if (numericValues.length === rawValues.length && numericValues.length > 0) {
    numericStats[col] = {
...
<head>
  <meta charset="UTF-8">
  <title>K-POP Groups Report</title>
  <style>
    body { font-family: sans-serif; padding: 2rem; max-width: 1200px; margin: auto; background: #f5f5f5; }
...
<body>
  <h1>Отчёт о K-POP группах</h1>
  <p>Всего групп: <strong>${records.length}</strong></p>
  ${numericTable}
  ${categoricalTables}
</body>
</html>`;
